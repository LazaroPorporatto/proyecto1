...
export class PresentacionService {
  ...
  async update(id: number, dto: UpdatePresentacionDto) {
    ...
    ensureNotSistemaEntity(presentacion, 'Presentación');
  ...
  async findAllFor(
    ...
    const data: PresentacionDto[] = result.map((p) => PresentacionMapper.toDto(p));
    return {
      data,
      total: 1,
    };
  ...
  async findBy(
    ...
    const data: PresentacionDto[] = result.data.map((p) =>
      PresentacionMapper.toDto(p),
    );
    return {
      data,
  ...
  async remove(id: number, usuarioId: number) {
    ...
    ensureNotSistemaEntity(entity, 'Presentación');
